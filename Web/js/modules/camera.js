...
async function loadFaceMesh() {
    ...
    if (typeof FaceMesh === 'undefined') {
        console.warn('MediaPipe FaceMesh 未載入，將使用簡化版臉部偵測');
        return;
    }
    ...
    faceMesh.setOptions({
        ...
        minDetectionConfidence: 0.3,  // 降低至0.3，更容易偵測臉部
        minTrackingConfidence: 0.3    // 降低至0.3，提高偵測寬容度
    });
...
function checkFaceAlignment(landmarks) {
    ...
    const idealCenterY = 0.45;
    const idealSize = 0.25;  // 理想眼距佔畫面比例

    const toleranceX = 0.25;   // 增加至0.25，允許更多水平偏移
    const toleranceY = 0.25;   // 增加至0.25，允許更多垂直偏移
    const toleranceSize = 0.15; // 增加至0.15，允許更多尺寸變化

    // 檢查位置
    const isXAligned = Math.abs(centerX - idealCenterX) < toleranceX;
    const isYAligned = Math.abs(centerY - idealCenterY) < toleranceY;
...
function detectBrightness() {
    ...
    const width = 100;  // 取樣寬度
    const height = 100; // 取樣高度
